...
{
    // prepare command line arguments and pass to monedad
    ...
    qDebug() << "starting monedad " + m_monedad;
    ...
    // Start monedad
    bool started = m_daemon->startDetached(m_monedad, arguments);
    ...
    while(true && !m_app_exit) {
        ...
        if(running(testnet)) {
            ...
            if(counter >= 5) {
                qDebug() << "Killing it! ";
#ifdef Q_OS_WIN
                QProcess::execute("taskkill /F /IM monedad.exe");
#else
                QProcess::execute("pkill monedad");
#endif
            }
...
{ 
    ...
    qDebug() << status;
    // `./monedad status` returns BUSY when syncing.
...
bool DaemonManager::sendCommand(const QString &cmd,bool testnet, QString &message) const
{
    ...
    p.start(m_monedad, external_cmd);
    ...
    : QObject(parent)
{
    ...
    // Platform depetent path to monedad
#ifdef Q_OS_WIN
    m_monedad = QApplication::applicationDirPath() + "/monedad.exe";
#elif defined(Q_OS_UNIX)
    m_monedad = QApplication::applicationDirPath() + "/monedad";
#endif

    if (m_monedad.length() == 0) {
