...
class WalletListenerImpl : public  Moneda::WalletListener
{
public:
...
{
    QFuture<Moneda::Wallet::ConnectionStatus> future = QtConcurrent::run(m_walletImpl, &Moneda::Wallet::connected);
    QFutureWatcher<Moneda::Wallet::ConnectionStatus> *connectionWatcher = new QFutureWatcher<Moneda::Wallet::ConnectionStatus>();

    connect(connectionWatcher, &QFutureWatcher<Moneda::Wallet::ConnectionStatus>::finished, [=]() {
        QFuture<Moneda::Wallet::ConnectionStatus> future = connectionWatcher->future();
...
{
    Moneda::PendingTransaction * ptImpl = m_walletImpl->createTransaction(
                dst_addr.toStdString(), payment_id.toStdString(), amount, mixin_count,
                static_cast<Moneda::PendingTransaction::Priority>(priority));
...
{
    Moneda::PendingTransaction * ptImpl = m_walletImpl->createTransaction(
                dst_addr.toStdString(), payment_id.toStdString(), Moneda::optional<uint64_t>(), mixin_count,
                static_cast<Moneda::PendingTransaction::Priority>(priority));
...
{
    Moneda::PendingTransaction * ptImpl = m_walletImpl->createSweepUnmixableTransaction();
    PendingTransaction * result = new PendingTransaction(ptImpl, this);
    return result;
}
...
UnsignedTransaction * Wallet::loadTxFile(const QString &fileName)
{
    ...
    Moneda::UnsignedTransaction * ptImpl = m_walletImpl->loadUnsignedTx(fileName.toStdString());
...
{
    return QString::fromStdString(Moneda::Wallet::genPaymentId());
}
...
{
    return QString::fromStdString(m_walletImpl->getDefaultDataDir()) + "/bitmoneda.log";
}

QString Wallet::getWalletLogPath() const
{
    return QCoreApplication::applicationDirPath() + "/moneda-wallet-gui.log";
}

Wallet::Wallet(Moneda::Wallet *w, QObject *parent)
    : QObject(parent)
    , m_walletImpl(w)
    , m_history(nullptr)
...
Wallet::~Wallet()
{
    ...
    delete m_history;
    m_history = NULL;
    //Moneda::WalletManagerFactory::getWalletManager()->closeWallet(m_walletImpl);
