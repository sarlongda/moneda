                                    ...
                                    const QString &language, bool testnet)
{
    ...
    Moneda::Wallet * w = m_pimpl->createWallet(path.toStdString(), password.toStdString(),
...
Wallet *WalletManager::openWallet(const QString &path, const QString &password, bool testnet)
{
    ...
    Moneda::Wallet * w =  m_pimpl->openWallet(path.toStdString(), password.toStdString(), testnet);
...
Wallet *WalletManager::recoveryWallet(const QString &path, const QString &memo, bool testnet, quint64 restoreHeight)
{
    ...
    Moneda::Wallet * w = m_pimpl->recoveryWallet(path.toStdString(), memo.toStdString(), testnet, restoreHeight);
    m_currentWallet = new Wallet(w);
    return m_currentWallet;
}
                                            ...
                                            quint64 restoreHeight)
{
    ...
    Moneda::Wallet * w = m_pimpl->createWalletFromKeys(path.toStdString(), language.toStdString(), testnet, restoreHeight,
...
{
    return Moneda::Wallet::maximumAllowedAmount();
}
...
{
    return QString::fromStdString(Moneda::Wallet::displayAmount(amount));
}

quint64 WalletManager::amountFromString(const QString &amount) const
{
    return Moneda::Wallet::amountFromString(amount.toStdString());
}

quint64 WalletManager::amountFromDouble(double amount) const
{
    return Moneda::Wallet::amountFromDouble(amount);
}

bool WalletManager::paymentIdValid(const QString &payment_id) const
{
    return Moneda::Wallet::paymentIdValid(payment_id.toStdString());
}

bool WalletManager::addressValid(const QString &address, bool testnet) const
{
    return Moneda::Wallet::addressValid(address.toStdString(), testnet);
}

bool WalletManager::keyValid(const QString &key, const QString &address, bool isViewKey,  bool testnet) const
{
    std::string error;
    if(!Moneda::Wallet::keyValid(key.toStdString(), address.toStdString(), isViewKey, testnet, error)){
...
{
    return QString::fromStdString(Moneda::Wallet::paymentIdFromAddress(address.toStdString(), testnet));
}
...
{
    Moneda::WalletManagerFactory::setLogLevel(logLevel);
}

void WalletManager::setLogCategories(const QString &categories)
{
    Moneda::WalletManagerFactory::setLogCategories(categories.toStdString());
}
...
QString WalletManager::checkUpdates(const QString &software, const QString &subdir) const
{
  ...
  const std::tuple<bool, std::string, std::string, std::string, std::string> result = Moneda::WalletManager::checkUpdates(software.toStdString(), subdir.toStdString());
  if (!std::get<0>(result))
    return QString("");
...
{
    m_pimpl =  Moneda::WalletManagerFactory::getWalletManager();
}
