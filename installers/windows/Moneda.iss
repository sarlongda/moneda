; Moneda Helium Hydra GUI Wallet Installer for Windows
; Copyright (c) 2014-2017, The Moneda Project
; See LICENSE

[Setup]
AppName=Moneda GUI Wallet
; For InnoSetup this is the property that uniquely identifies the application as such
; Thus it's important to keep this stable over releases
; With a different "AppName" InnoSetup would treat a mere update as a completely new application and thus mess up

AppVersion=0.11.1.0
DefaultDirName={pf}\Moneda GUI Wallet
DefaultGroupName=Moneda GUI Wallet
UninstallDisplayIcon={app}\moneda-wallet-gui.exe
...
; As of version 0.11.1.0, the Moneda .exe files do not carry version info anyway in their .exe headers.
...
; applications somehow. Luckily this is no issue here because ALL files are "private" to Moneda.

Source: "ReadMe.htm"; DestDir: "{app}"; Flags: ignoreversion
Source: "FinishImage.bmp"; Flags: dontcopy

; Moneda GUI wallet
Source: "bin\moneda-wallet-gui.exe"; DestDir: "{app}"; Flags: ignoreversion

; Moneda GUI wallet log file
...
Source: "moneda-wallet-gui.log"; DestDir: "{app}"; Flags: onlyifdoesntexist; Permissions: users-modify

; Moneda CLI wallet
Source: "bin\moneda-wallet-cli.exe"; DestDir: "{app}"; Flags: ignoreversion

; Moneda wallet RPC interface implementation
Source: "bin\moneda-wallet-rpc.exe"; DestDir: "{app}"; Flags: ignoreversion

; Moneda daemon
Source: "bin\monedad.exe"; DestDir: "{app}"; Flags: ignoreversion

; Moneda daemon wrapped in a batch file that stops before the text window closes, to see any error messages
Source: "moneda-daemon.bat"; DestDir: "{app}"; Flags: ignoreversion;

; Moneda blockchain utilities
Source: "bin\moneda-blockchain-export.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\moneda-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion

; was present in 0.10.3.1, not present anymore in 0.11.1.0
; Source: "bin\moneda-utils-deserialize.exe"; DestDir: "{app}"; Flags: ignoreversion
...
; Filename: "{app}\moneda-wallet-gui.exe"; Description: "Run GUI Wallet now"; Flags: postinstall nowait skipifsilent
...
begin
  ...
  blockChainDefaultDir := ExpandConstant('{commonappdata}\bitmoneda');
  s := 'The default folder to store the Moneda blockchain is ' + blockChainDefaultDir;
...
begin
  Result := BlockChainDir('') + '\bitmoneda.log';
...
begin
  if CurStep = ssPostInstall then begin
    // Re-build "moneda-daemon.bat" according to actual install and blockchain directory used
    SetArrayLength(s, 3);
    s[0] := 'REM Execute the Moneda daemon and then stay with window open after it exits';
    s[1] := '"' + ExpandConstant('{app}\monedad.exe') + '" ' + DaemonFlags('');
    s[2] := 'PAUSE';
    SaveStringsToFile(ExpandConstant('{app}\moneda-daemon.bat'), s, false); 
  end;
end;
...
[Icons]
; Icons in the "Moneda GUI Wallet" program group
; Windows will almost always display icons in alphabetical order, per level, so specify the text accordingly
Name: "{group}\GUI Wallet"; Filename: "{app}\moneda-wallet-gui.exe"
...
Name: "{group}\Utilities\Moneda Daemon"; Filename: "{app}\monedad.exe"; Parameters: {code:DaemonFlags}
...
Name: "{group}\Utilities\Textual (CLI) Wallet"; Filename: "{app}\moneda-wallet-cli.exe"; WorkingDir: "{userdocs}\Moneda\wallets"
...
Name: "{group}\Utilities\x (Check Default Wallet Folder)"; Filename: "{win}\Explorer.exe"; Parameters: "{userdocs}\Moneda\wallets"
Name: "{group}\Utilities\x (Check GUI Wallet Log)"; Filename: "Notepad"; Parameters: "{app}\moneda-wallet-gui.log"
Name: "{group}\Utilities\x (Try Daemon, Exit Confirm)"; Filename: "{app}\moneda-daemon.bat"
Name: "{group}\Utilities\x (Try GUI Wallet Low Graphics Mode)"; Filename: "{app}\start-low-graphics-mode.bat"
Name: "{group}\Utilities\x (Try Kill Daemon)"; Filename: "Taskkill.exe"; Parameters: "/IM monedad.exe /T /F"

; Desktop icons, optional with the help of the "Task" section
Name: "{userdesktop}\GUI Wallet"; Filename: "{app}\moneda-wallet-gui.exe"; Tasks: desktopicon
...
Root: HKCU; Subkey: "Software\moneda-project"; Flags: uninsdeletekeyifempty
Root: HKCU; Subkey: "Software\moneda-project\moneda-core"; Flags: uninsdeletekey
Root: HKCU; Subkey: "Software\moneda-project\moneda-core"; ValueType: string; ValueName: "daemonFlags"; ValueData: {code:DaemonFlags};
