...
Core tests take longer than any other Moneda tests, due to the high amount of computational work involved in validating core components.
...
To run only Moneda's core tests (after building):
...
To run only Moneda's crypto tests (after building):
...
To run only Moneda's hash tests (after building):
...
To run only Moneda's performance tests (after building):
...
To run only Moneda's unit tests (after building):
