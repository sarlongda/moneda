...
# Base environnement
        ...
        cd moneda/utils/build_scripts
        docker build -f android32.Dockerfile -t moneda-android .
        cd ..

# Build GUI

        cd android/docker
        docker build -t moneda-gui-android .
        docker create -it --name moneda-gui-android moneda-gui-android bash

# Get the apk

        docker cp moneda-gui-android:/opt/android/moneda-gui/build/release/bin/bin/QtApp-debug.apk .
...
- Using ADB (Android debugger bridge) : 
  ...
  The only place where we are allowed to play is `/data/local/tmp`. So : 
        ...
        adb  push /opt/android/moneda-gui/build/release/bin/bin/QtApp-debug.apk /data/local/tmp
