...
if [ "$platform" == "darwin" ]; then
    BIN_PATH=$BIN_PATH/moneda-wallet-gui.app/Contents/MacOS/
elif [ "$platform" == "mingw64" ] || [ "$platform" == "mingw32" ]; then
    MONEDAD_EXEC=monedad.exe
fi
...
get_tag
popd
echo "var GUI_MONEDA_VERSION = \"$TAGNAME\"" >> version.js
...
$QMAKE ../moneda-wallet-gui.pro "$CONFIG" || exit
$MAKE || exit 

# Copy monedad to bin folder
if [ "$platform" != "mingw32" ] && [ "$ANDROID" != true ]; then
cp ../$MONEDA_DIR/bin/$MONEDAD_EXEC $BIN_PATH
fi
